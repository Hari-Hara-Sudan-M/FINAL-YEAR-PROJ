# =============================================================================
# Temperature Sensitivity Experiment (Table V)
#
# Runs FeCoGraph with different TEMPERATURE values: 0.1, 0.3, 0.5, 0.7, 0.9
# Each run saves to results/te mp_X/. Does NOT modify config.py.
#
# Usage:  python run_temperature.py
# =============================================================================
...
TEMP_VALUES = [0.1, 0.3, 0.5, 0.7]  # Paper Table V exact values
BASE_PORT = 9980
...
def server_worker(port, result_log_dir, result_ckpt_dir, temp_val):
    log = setup_logger(f"SRV-T{temp_val}", os.path.join(result_log_dir, "server.log"))
    ...
    log.info(f"Temp={temp_val} Server on port {port}")
    ...
    log.info(f"DONE | Temp={temp_val} | BMTA={bmta_p:.4f}")
...
def run_one(temp_val, port):
    cfg.TEMPERATURE = temp_val
    tag = str(temp_val).replace(".", "p")
    rdir = os.path.join("results", f"temp_{tag}")
    ...
    print(f"\n{'='*70}\nRunning TEMPERATURE = {temp_val} | Results -> {rdir}\n{'='*70}")
    st = threading.Thread(target=server_worker, args=(port, rlog, rckpt, temp_val), daemon=True)
    ...
    print(f"Temp={temp_val} DONE!\n")


def generate_summary():
    import pandas as pd
    print("\n" + "=" * 70)
    print("TEMPERATURE SENSITIVITY SUMMARY (Paper Table V)")
    print("=" * 70)
    rows = []
    for tv in TEMP_VALUES:
        tag = str(tv).replace(".", "p")
        hp = os.path.join("results", f"temp_{tag}", "logs", "training_history.csv")
        if os.path.exists(hp):
            df = pd.read_csv(hp)
            bmta = df["mean_acc_pers"].max()
            best_f1 = df["mean_f1_pers"].max()
            final = df.iloc[-1]
            rows.append({"temperature": tv, "BMTA": round(bmta*100, 2),
                         "Best_F1": round(best_f1*100, 2),
                         "Final_Acc": round(final["mean_acc_pers"]*100, 2),
                         "Final_F1": round(final["mean_f1_pers"]*100, 2)})
            print(f"  tau={tv:<5} | BMTA={bmta*100:.2f}% | F1={best_f1*100:.2f}%")
    if rows:
        summary_path = os.path.join("results", "temperature_summary.csv")
...
if __name__ == "__main__":
    print("=" * 70)
    print("FeCoGraph Temperature Sensitivity (Paper Table V - Multiclass)")
    print(f"Values: {TEMP_VALUES}")
    print("=" * 70)
    for i, tv in enumerate(TEMP_VALUES):
        run_one(tv, BASE_PORT + i)
    generate_summary()
    print("\nALL TEMPERATURE EXPERIMENTS DONE! Results in: results/temp_*/")
